...
public class Tarefa3 {
  public static void main(String[] args) {
    ...
    for (int i = 1; i <= 100 && dirName == ""; i++) {
      String dir = "./resultados/tarefa3/teste" + i;
      try {
    ...
    while (N <= 10000) {
      ...
      for (int i = 0; i < N * N; i++) {
        ...
        if (gerador.nextBoolean())
          ...
          arvoreBB.remover2(valorRemover);
        if (gerador.nextBoolean())
          arvoreAVL.remover(valorRemover);
        else
          arvoreAVL.remover2(valorRemover);
      ...
      // Imprimir o tamanho das árvores
      System.out.println("\nN = " + N);
      ...
      N = (int) Math.ceil(N * (Math.pow(10000 / (double) 100, 1.0 / (5 - 1))));
      N = Math.min(N, 9000); // Garantir que N não ultrapasse o limite máximo
    }
  }
