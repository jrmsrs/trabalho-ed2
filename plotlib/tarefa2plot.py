...
def plot(bst_heights, avl_heights, N_values, bst_dp=None, avl_dp=None):
    ...
    plt.text(1000, 17, "Esperado: se manter na altura original")
    ...
    plt.xlim(100, 10000)
    # plt.show()
    plt.savefig("tarefa2.png")

bst_heights = []
avl_heights = []
N_value = []

for i, dir in enumerate(os.listdir("resultados/tarefa2/")):
    with open(f"resultados/tarefa2/teste{i+1}/alturasBB.txt", "r") as file:
    ...
    with open(f"resultados/tarefa2/teste{i+1}/alturasAVL.txt", "r") as file:
        avl_height = [int(height) for height in file.readlines()]
        avl_heights.append(avl_height)
    with open(f"resultados/tarefa2/teste{i+1}/amostra.txt", "r") as file:
